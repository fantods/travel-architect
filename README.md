...
1.  *Step 1:* LLM calls `getWeather` tool with `{ city: "London" }`.
2.  *Step 2:* Tool returns "Rain on Sunday".
3.  *Step 3:* LLM calls `searchPlaces` with `{ location: "London", category: "museum" }`.
4.  *Step 4:* LLM determines the plan is ready.
5.  *Final Output:* Returns the JSON object matching `ItinerarySchema`.
